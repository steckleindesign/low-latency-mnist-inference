...
/*
    ...
    3) Valididate 84 downstream neurons serially, 3 neurons valid every 4 cycles
...
module fc_tri_structure_adder_tree (
    input  logic       i_clk,
    input  logic       i_rst,
    ...
    // [1+2] 3
    logic [7:0] adder1_stage1[0:89], adder2_stage1[0:59], adder3_stage1[0:29];
    logic [7:0] adder1_stage2[0:74], adder2_stage2[0:89], adder3_stage2[0:104];
    logic [7:0] adder1_stage3[0:37], adder2_stage3[0:44], adder3_stage3[0:52];
    logic [7:0] adder1_stage4[0:18], adder2_stage4[0:22], adder3_stage4[0:26];
    logic [7:0] adder1_stage5[0:9],  adder2_stage5[0:11], adder3_stage5[0:13];
    logic [7:0] adder1_stage6[0:4],  adder2_stage6[0:5],  adder3_stage6[0:6];
    logic [7:0] adder1_stage7[0:2],  adder2_stage7[0:2],  adder3_stage7[0:3];
    logic [7:0] adder1_stage8[0:1],  adder2_stage8[0:1],  adder3_stage8[0:1];
    logic [7:0] adder1_result,       adder2_result,       adder3_result;
    ...
    always_ff @(posedge i_clk) begin
        if (~i_rst) done <= 1'b0;
        else done <= (feature_ctr == NUM_FEATURES-1) && (neuron_ctr == NUM_NEURONS-1);
    end
    ...
    always_ff @(posedge i_clk) begin
        ...
        adder1_stage1[ 0:29] <= Preg[ 0:29];
        adder1_stage1[30:59] <= Preg[30:59];
        adder1_stage1[60:89] <= Preg[60:89];
        
        adder1_stage2[0:29] <= Preg[60:89];
        for (int i = 0; i < 45; i++)
        ...
        adder2_stage1[0:29] <= Preg[0:29];
        adder2_stage1[30:59] <= Preg[30:59];
        
        adder2_stage2[0:29] <= Preg[30:59];
        adder2_stage2[30:59] <= Preg[60:89];
        for (int i = 0; i < 30; i++)
        ...
        adder3_stage1 <= Preg[0:29];
        
        adder3_stage2[0:29] <= Preg[0:29];
        adder3_stage2[30:59] <= Preg[30:59];
        adder3_stage2[60:89] <= Preg[60:89];
        for (int i = 0; i < 15; i++)
